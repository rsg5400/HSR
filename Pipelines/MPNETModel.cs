...
using NLP.Tokenizers.WordPiece.MPNETTokenizers;
...
namespace NLP.Pipelines
{
    public class MPNETModel
    {
        private readonly MPNETModelConfig _mpNetModelCofiguration;
        private readonly MPNetTokenizer _tokenizer;
     
        public MPNETModel(MPNETModelConfig mpNetModelCofiguration)
        {
            _mpNetModelCofiguration = mpNetModelCofiguration;
            //_bertModelConfiguration.CreateSession();
            _tokenizer = new MPNetTokenizer(_mpNetModelCofiguration);
        }
        ...
        public float[] Predict(params string[] sentence)
        {
            ...
            var modelOutputTensor = _mpNetModelCofiguration.Run(input).First().AsTensor<float>(); 
